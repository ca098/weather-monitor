...
class EmailService:
    def __init__(self, caching_service: CachingService, mysql_service: MySqlService, weather_service: WeatherService, config: _Environ[str]):
    ...
    def run_mail_service(self) -> None:
        ...
        scheduler.add_job(func=self.collect_data_and_send_mail, trigger="interval", seconds=refresh_rate)
        scheduler.start()
        ...
        for loc, val in locations.items():
            ...
            for user in users_for_location:
                ...
                if ws > user.wind_speed_exceeds:
                    subject, mail_text = self.create_mail_text(user, "wind speed exceeds", user.wind_speed_exceeds, wd["wind"]["speed"])
                    self.send_mail(subject, mail_text, user.email, user.location)

                if wc == user.weather_code:
                    weather_str = self.weather_service.get_weather_for_code(user.weather_code)
                    subject, mail_text = self.create_mail_text(user, "weather type equals", weather_str, weather_str)
                    self.send_mail(subject, mail_text, user.email, user.location)

                if wt > user.temp_celsius_above:
                    subject, mail_text = self.create_mail_text(user, "temperature exceeds", user.temp_celsius_above, wt)
                    self.send_mail(subject, mail_text, user.email, user.location)
    ...
    def create_mail_text(self, user: SubscriptionList, metric: str, user_value, weather_value) -> tuple[str, str]:
