...
@WEATHER_BLUEPRINT.route('/api/v1/create_subscription/<string:email>/<string:location>', methods=['POST'])
...
def create_new_subscription(email: str, location: str):
    ...
    req = request.json
    create_subscription = SE.weather_service.create_new_subscription(email=email, location=location.lower(), params=req)

    if "error" in create_subscription:
        return jsonify(create_subscription), 400

    return jsonify(create_subscription), 201


@WEATHER_BLUEPRINT.route('/api/v1/update_subscription/<string:email>/<string:location>', methods=['POST'])
...
def update_subscriptions(email: str, location: str):
    ...
    req = request.json
    update_subscription = SE.weather_service.update_subscription(email=email, location=location.lower(), params=req)
...
def list_subscriptions(email: str):
    ...
    return jsonify(SE.weather_service.get_subscription_list(email)), 200
...
@WEATHER_BLUEPRINT.route('/api/v1/delete_subscription/<string:email>/<string:location>', methods=['DELETE'])
@swag_from('swagger/delete_subscription.yml')
def delete_subscriptions(email: str, location: str):
    if not is_valid_email(email):
        return jsonify({"message": INVALID_EMAIL_MESSAGE}), 400

    removed_subscription = SE.weather_service.delete_subscription(email=email, location=location.lower())
    if "error" in removed_subscription:
        return jsonify(removed_subscription), 404

    return jsonify(removed_subscription), 200
